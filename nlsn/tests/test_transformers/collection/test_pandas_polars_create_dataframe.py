...
class TestFromDictOfListToListOfDicts:
    ...
    def test_valid_input(self):
        ...
        assert exp == FromData._from_dict_of_list_to_list_of_dicts(data)

    def test_empty_input(self):
        """Empty data."""
        data = {"a": [], "b": []}
        assert [] == FromData._from_dict_of_list_to_list_of_dicts(data)

    def test_mismatched_lengths(self):
        """Wrong lengths."""
        data = {"a": [1, 2], "b": ["x", "y", "z"]}
        with pytest.raises(ValueError):
            FromData._from_dict_of_list_to_list_of_dicts(data)
...
def test_create_dataframe_pandas_polars(df_input, backend, kwargs, key):
    ...
    t = FromData(data=data, storage_key=key, kwargs=kwargs)
    ns.clear()
    try:
