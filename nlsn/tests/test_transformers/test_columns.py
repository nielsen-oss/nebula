...
from nlsn.nebula.transformers import DropColumns, SelectColumns
...
class TestDropColumns:
    ...
    def test_drop_columns(self, spark, backend, is_nw, kws):
    ...
    def test_drop_columns_not_present(self):
        ...
        df_input = pd.DataFrame([[1, 2, ], [3, 4]], columns=["a", "b"])
...
class TestSelectColumns:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("polars", True, {"glob": "*"}),
            ("spark", False, {"regex": "c[23]"})
        ]
    )
    def test_select_columns(self, spark, backend, is_nw, kws):
        """Test several combinations."""
        data = _get_random_int_data(5, len(_columns_product))
        df_input = pd.DataFrame(data, columns=_columns_product)
        input_columns: list[str] = df_input.columns.tolist()
