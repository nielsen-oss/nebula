...
A loop must be defined under a "loop" key with the following structure:
    {
        "loop": {
            ...
            "generation_type": "linear"|"product",  # Optional, defaults to "linear"
...
def validate_loop_params(d: dict) -> None:
    ...
    Raises:
        TypeError: If values or generation_type are of the wrong type
        ValueError: If values have different lengths or invalid generation_type
    ...
    gen_type = d.get("generation_type")
    if gen_type is not None:
        if not isinstance(gen_type, str):
            raise TypeError("The 'generation_type' must be a string.")
        if gen_type not in {"linear", "product"}:
            raise ValueError("The 'generation_type' must be 'linear' or 'product'.")
...
def _explode_loops(o) -> Tuple[Any, bool]:
    ...
    if isinstance(o, dict):
        ...
        for k, value in o.items():
            ret[k], _ = _explode_loops(value)

        if "loop" in o:
            loop_content = o["loop"]
            # Process any nested loops in the loop content first
            loop_content, _ = _explode_loops(loop_content)
        ...
        ret = []
        for value in o:
            res, is_exploded = _explode_loops(value)
            if is_exploded and isinstance(res, list):
                ret.extend(res)
            else:
                ret.append(res)
    else:
...
def explode_loops_in_pipeline(o: dict) -> dict:
    ...
    if is_split:
        for k, item in pipe.items():
            processed, _ = _explode_loops(item)
            ret["pipeline"][k] = processed
    else:
        for item in pipe:
            processed, is_exploded = _explode_loops(item)
            if is_exploded and isinstance(processed, list):
