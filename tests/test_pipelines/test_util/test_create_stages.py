...
from nebula.pipelines.pipelines import TransformerPipeline, _create_stages
from nebula.transformers import Cache, AssertNotEmpty, LogDataSkew, SelectColumns
...
def test_create_stages_nested(add_start):
    ...
    pipe_test = TransformerPipeline(
        [AssertNotEmpty()], interleaved=Cache(), prepend_interleaved=False, name="pipe_test"
    )
